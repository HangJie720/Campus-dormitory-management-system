...
<script type="text/javascript">
	function _go() {
		...
		location = "BuildingManager.action?currentPage=" + currentPage+"&SearchKey=${SearchKey}";
	}
</script>
...
<c:choose>
	...
	<c:otherwise><a href="BuildingManager.action?currentPage=1&SearchKey=${SearchKey}" class="aBtn bold">首页</a></c:otherwise>
</c:choose>
<c:choose>
	<c:when test="${currentPage eq 1 }"><span class="spanBtnDisabled">上一页</span></c:when>
	<c:otherwise><a href="BuildingManager.action?currentPage=${currentPage-1}&SearchKey=${SearchKey}" class="aBtn bold">上一页</a></c:otherwise>
</c:choose>
...
4. 如果end>tp, 让begin=tp-5, end=tp
 ...
 <c:forEach begin="${begin }" end="${end }" var="i">
   <c:choose>
   	  ...
   	  <c:otherwise>
   	    <a href="BuildingManager.action?currentPage=${i}&SearchKey=${SearchKey}" class="aBtn">${i }</a>
   	  </c:otherwise>
   </c:choose>
...
<c:choose>
	...
	<c:otherwise><a href="BuildingManager.action?currentPage=${currentPage+1}&SearchKey=${SearchKey}" class="aBtn bold">下一页</a></c:otherwise>
</c:choose>
<c:choose>
	<c:when test="${currentPage eq pageCount }"><span class="spanBtnDisabled">尾页</span></c:when>
	<c:otherwise><a href="BuildingManager.action?currentPage=${pageCount}&SearchKey=${SearchKey}" class="aBtn bold">尾页</a></c:otherwise>
</c:choose>      

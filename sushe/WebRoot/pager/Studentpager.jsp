...
<script type="text/javascript">
	function _go() {
		...
		location = "StudentManager.action?currentPage=" + currentPage+"&SearchRow=${SearchRow }&SearchKey=${SearchKey}&State=${State}" ;
	}
</script>
...
<c:choose>
	...
	<c:otherwise><a href="StudentManager.action?currentPage=1&SearchRow=${SearchRow }&SearchKey=${SearchKey}&State=${State}" class="aBtn bold">首页</a></c:otherwise>
</c:choose>
<c:choose>
	<c:when test="${currentPage eq 1 }"><span class="spanBtnDisabled">上一页</span></c:when>
	<c:otherwise><a href="StudentManager.action?currentPage=${currentPage-1}&SearchRow=${SearchRow }&SearchKey=${SearchKey}&State=${State}" class="aBtn bold">上一页</a></c:otherwise>
</c:choose>
...
4. 如果end>tp, 让begin=tp-5, end=tp
 ...
 <c:forEach begin="${begin }" end="${end }" var="i">
   <c:choose>
   	  ...
   	  <c:otherwise>
   	    <a href="StudentManager.action?currentPage=${i}&SearchRow=${SearchRow }&SearchKey=${SearchKey}&State=${State}" class="aBtn">${i }</a>
   	  </c:otherwise>
   </c:choose>
...
<c:choose>
	...
	<c:otherwise><a href="StudentManager.action?currentPage=${currentPage+1}&SearchRow=${SearchRow }&SearchKey=${SearchKey}&State=${State}" class="aBtn bold">下一页</a></c:otherwise>
</c:choose>
<c:choose>
	<c:when test="${currentPage eq pageCount }"><span class="spanBtnDisabled">尾页</span></c:when>
	<c:otherwise><a href="StudentManager.action?currentPage=${pageCount}&SearchRow=${SearchRow }&SearchKey=${SearchKey}&State=${State}" class="aBtn bold">尾页</a></c:otherwise>
</c:choose>      

...
<script type="text/javascript">
	function _go() {
		...
		location = "OutList.action?currentPage=" + currentPage;
	}
</script>
...
<c:choose>
	...
	<c:otherwise><a href="OutList.action?currentPage=1" class="aBtn bold">首页</a></c:otherwise>
</c:choose>
<c:choose>
	<c:when test="${currentPage eq 1 }"><span class="spanBtnDisabled">上一页</span></c:when>
	<c:otherwise><a href="OutList.action?currentPage=${currentPage-1}" class="aBtn bold">上一页</a></c:otherwise>
</c:choose>
...
4. 如果end>tp, 让begin=tp-5, end=tp
 ...
 <c:forEach begin="${begin }" end="${end }" var="i">
   <c:choose>
   	  ...
   	  <c:otherwise>
   	    <a href="OutList.action?currentPage=${i}" class="aBtn">${i }</a>
   	  </c:otherwise>
   </c:choose>
...
<c:choose>
	...
	<c:otherwise><a href="OutList.action?currentPage=${currentPage+1}" class="aBtn bold">下一页</a></c:otherwise>
</c:choose>
<c:choose>
	<c:when test="${currentPage eq pageCount }"><span class="spanBtnDisabled">尾页</span></c:when>
	<c:otherwise><a href="OutList.action?currentPage=${pageCount}" class="aBtn bold">尾页</a></c:otherwise>
</c:choose>      

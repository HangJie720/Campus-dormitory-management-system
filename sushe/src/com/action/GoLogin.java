	...
	public String execute() throws Exception {
		...
		if(Type.equals("系统管理员"))
		{
			...
			else
			{
				...
				String Admin_Name = new AdminDao().getAdminName(Admin_ID);
				//创建session
		...
		else if(Type.equals("楼宇管理员"))
		{
			...
			else
			{
				...
				String Teacher_Name=new TeacherDao().GetTeacherName(Teacher_ID);
		
				//创建session
				HttpSession session = ServletActionContext.getRequest().getSession();
				session.setAttribute("teachername", Teacher_Name);
		...
		else if(Type.equals("学生"))
		{
			...
			else
			{
				...
				String Student_Name= new StudentDao().GetStudentName(Student_ID);
				//创建session
				HttpSession session = ServletActionContext.getRequest().getSession();
				session.setAttribute("studentname", Student_Name);
